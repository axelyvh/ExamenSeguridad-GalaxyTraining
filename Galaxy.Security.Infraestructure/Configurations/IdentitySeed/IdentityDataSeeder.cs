...
namespace Galaxy.Security.Infraestructure.Configurations.IdentitySeed
{
    public static class IdentityDataSeeder
    {
        public static async Task SeedAsync(IServiceProvider service)
        {
            ...
            var mangerUser = new UserExtension
            {
                FullName = "Jhon Chavez",
                UserName = "jchavez",
                Email = "jhonchavez@gmail.com",
                EmailConfirmed = true,
                UserId = new("4fa2b6f5-1c3b-4e2e-8f0a-2c3b5f6e7d7a")
            };

            var result1 = await userManager.CreateAsync(mangerUser, "Password2025");
            if (result1.Succeeded)
            {
                await userManager.AddToRoleAsync(mangerUser, RolesConstants.ManagerRole);
            }

            var customerUser = new UserExtension
            {
                FullName = "Axel Vasquez",
                UserName = "avasquez",
                Email = "avasquez@gmail.com",
                EmailConfirmed = true,
                UserId = new("4fa2b6f5-1c3b-4e2e-8f0a-2c3b5f6e7d6a")
            };
